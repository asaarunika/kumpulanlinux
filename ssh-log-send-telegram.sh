...
#### Setting TelegramBot ####
apikey="786516894:AAEg1KqdtdMy1MmASfRitlGrOYYkZpd_0A4" # Api key telegram
chatid="-1001459150571" # Chat id telegram
#############################
...
#  GETINFO=$(curl -Ls "http://ip-api.com/json/${IP}" -A "Mozilla/5.0 buntu; Linux x86_64; rv:70.0) Gecko/20100101 Firefox/70.0")
  GETIP=$(echo $GETINFO | grep -Po '(?<=IP:</th><td>)[^<]*')
  if [ -z "$GETIP" ]; then
    # Login SSH menggunakan IP Lokal
    curl -s "https://api.telegram.org/bot${apikey}/sendMessage?chat_id=${chatid}&parse_mode=Markdown&text=*NOTIFIKASI LOGIN SSH*%0d%0aUser : *${USER}*%0d%0aDate : *${DATE_LOG}*.%0d%0aIP : ${IP}%0d%0aPORT : ${PORT}%0d%0aServer : ${HOST}" > /dev/null
    echo "${IP} : ${PORT}" >> $LOG
  else
